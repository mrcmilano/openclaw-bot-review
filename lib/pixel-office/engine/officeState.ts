...
function getCodeSnippets(): string[] {
  return CODE_SNIPPETS_EN
}
function getSreBlackwords(): string[] {
  return SRE_BLACKWORDS_EN
}
function getPhotoComments(): string[] {
  return PHOTO_COMMENTS_EN
}
function getTempWorkerLabel(): string {
  return 'Temp'
}
function getGatewaySreLabel(): string {
  return 'On-Call SRE'
}
...
const GATEWAY_SRE_LABEL = 'On-Call SRE'
    ...
    for (const ch of this.characters.values()) {
      ...
      if (ch.isViewingPhoto && ch.state === CharacterState.IDLE && !ch.isCat && !ch.isLobster) {
        ...
        if (ch.photoComments.length < 2 && Math.random() < dt * PHOTO_COMMENT_SPAWN_RATE) {
          ch.photoComments.push({
            text: getPhotoComments()[Math.floor(Math.random() * getPhotoComments().length)],
            age: 0,
      ...
      if (isSreFirefighting && !ch.isCat && !ch.isLobster) {
        if (ch.codeSnippets.length < 3 && Math.random() < dt * SRE_BLACKWORD_SPAWN_RATE) {
          ch.codeSnippets.push({
            text: getSreBlackwords()[Math.floor(Math.random() * getSreBlackwords().length)],
            age: 0,
        ...
        if (ch.codeSnippets.length < 2 && Math.random() < dt * CODE_SNIPPET_SPAWN_RATE) {
          ch.codeSnippets.push({
            text: getCodeSnippets()[Math.floor(Math.random() * getCodeSnippets().length)],
            age: 0,
            x: (Math.random() - 0.5) * 20,
            y: 0,
          })
